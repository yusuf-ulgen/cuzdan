...
import com.yusufulgen.cuzdan.databinding.DialogDemandDepositCalculatorBinding
...
class DemandDepositCalculatorDialogFragment : DialogFragment() {

    private var _binding: DialogDemandDepositCalculatorBinding? = null
    private val binding get() = _binding!!

    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        _binding = DialogDemandDepositCalculatorBinding.inflate(LayoutInflater.from(requireContext()))

        val dialog = MaterialAlertDialogBuilder(requireContext(), R.style.CustomDialogTheme)
            .setTitle(getString(R.string.cash_tool_demand_deposit))
        ...
        dialog.setOnShowListener {
            ...
            btn.setOnClickListener {
                ...
                val annualRate = binding.editAnnualRate.text?.toString()?.trim().orEmpty().toBigDecimalOrNull() ?: BigDecimal.ZERO
                val stopajRate = binding.editStopajRate.text?.toString()?.trim().orEmpty().toBigDecimalOrNull() ?: BigDecimal.ZERO

                if (annualRate < BigDecimal.ZERO) {
                    binding.editAnnualRate.error = getString(R.string.error_invalid_rate)
                    return@setOnClickListener
                }
                if (stopajRate < BigDecimal.ZERO) {
                ...
                // Vadesiz mevduat: bankaya göre değişir ama kaba hesap için günlük basit faiz:
                ...
                binding.textResult.text = getString(
                    R.string.cash_demand_deposit_result_template_v2,
                    days.toString(),
