...
import com.yusufulgen.cuzdan.databinding.IncludeCashDemandDepositFormBinding
...
class DemandDepositCalculatorFragment : Fragment() {

    private var _base: FragmentCashCalculatorBaseBinding? = null
    private val base get() = _base!!

    private var _form: IncludeCashDemandDepositFormBinding? = null
    private val form get() = _form!!

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        _base = FragmentCashCalculatorBaseBinding.inflate(inflater, container, false)
        _form = IncludeCashDemandDepositFormBinding.inflate(inflater, base.contentContainer, true)
        return base.root
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        base.textTitle.text = getString(R.string.cash_tool_demand_deposit_short)
    ...
    private fun calculate() {
        ...
        val annualRate = form.editAnnualRate.text?.toString()?.trim().orEmpty().toBigDecimalOrNull() ?: BigDecimal.ZERO
        val stopajRate = form.editStopajRate.text?.toString()?.trim().orEmpty().toBigDecimalOrNull() ?: BigDecimal.ZERO

        if (annualRate < BigDecimal.ZERO) return
        if (stopajRate < BigDecimal.ZERO) return
